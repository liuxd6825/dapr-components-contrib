...
func NewMongoDB(componentName string, logger logger.Logger) *MongoDB {
	s := &MongoDB{
...
func (m *MongoDB) Init(metadata Metadata) error {
	...
	info := fmt.Sprintf("component: %s, dbtype:mongodb ,host:%s, server:%s, databaseName:%s", m.componentName, m.metadata.host, m.metadata.server, m.metadata.databaseName)
	m.logger.Info(info)
