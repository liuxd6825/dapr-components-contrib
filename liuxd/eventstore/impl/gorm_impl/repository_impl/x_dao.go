...
func (d *dao[T]) DeleteByWhere(ctx context.Context, where string, opts ...*Options) error {
	v := d.NewEntity()
	return d.getDB(ctx).Where(where).Delete(v).Error
}
...
func (d *dao[T]) UpdateByWhere(ctx context.Context, where string, setData map[string]any, opts ...*Options) error {
	return d.getDB(ctx).Where(where).Updates(setData).Error
}
