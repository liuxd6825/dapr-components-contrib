...
*/
func NewEvent(tenantId string, sessionId string, aggregateId string, aggregateType string, sequenceNumber uint64, dto *dto.EventDto) *model.Event {
	event := &model.Event{
		Id:             dto.EventId,
		SessionId:      sessionId,
		...
		IsSourcing:     dto.IsSourcing,
	}
	if len(sessionId) != 0 {
		event.SessionStatus = model.SessionStatusStart
	}
	return event
}
