...
import (
	...
	zeebe_test "github.com/liuxd6825/dapr-components-contrib/tests/certification/bindings/zeebe"
...
func TestThrowErrorOperation(t *testing.T) {
	...
	throwError := func(ctx flow.Context) error {
		client := zeebe_test.GetDaprClient(grpcPort)
		defer client.Close()
		...
		res, err := zeebe_test.ExecCommandOperation(ctx, client, bindings_zeebe_command.ThrowErrorOperation, data, nil)
		assert.NoError(t, err)
	...
	failForNotExistingJob := func(ctx flow.Context) error {
		...
		_, err = zeebe_test.ExecCommandOperation(ctx, client, bindings_zeebe_command.ThrowErrorOperation, data, nil)
		assert.Error(t, err)

		return nil
	}

	flow.New(t, "Test throw error operation").
		...
		Step("Throw an error", throwError).
