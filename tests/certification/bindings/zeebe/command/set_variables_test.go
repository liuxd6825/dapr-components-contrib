...
func TestSetVariablesOperation(t *testing.T) {
	...
	setVariables := func(ctx flow.Context) error {
		client := zeebe_test.GetDaprClient(grpcPort)
		defer client.Close()
	...
	failForNotExistingInstance := func(ctx flow.Context) error {
		client := zeebe_test.GetDaprClient(grpcPort)
		defer client.Close()
	...
	flow.New(t, "Test fail job operation").
		...
		Step("Set variables", setVariables).
		Step("Should fail for not existing instance", failForNotExistingInstance).
		Run()
}
